...
- **Library docs (TypeScript, npm)**: prefer Context7 MCP — `mcp__context7__resolve-library-id` then `mcp__context7__get-library-docs`. Training data for crate APIs often stale; Context7 live. Use WebFetch/WebSearch only when Context7 no cover or for blog posts, Anthropic docs, ecosystem think pieces.
