...
- **subsystems_touched** — match each path vs the module areas in
...
introducing new top-level module.
...
1. `/swarm-review` on 2-commit branch, 5 files in
   `src/views/` → tier=**high**, default overlays,
...
3. `/swarm-review 143` where PR #143 has labels
   `breaking-change` + `enhancement`, touches `src/webview/protocol`
...
4. `/swarm-review --base=v0.5.0` on branch 30 commits ahead
   ...
   gate).
5. `/swarm-review` with 4 files changed across `src/views/` and
   `src/webview/` → metrics say `low` (size) but `high` (≥2
   modules); low-confidence → meta-plan gate fires.