...
Read diff against resolved baseline (already computed by SKILL.md step 2). Identify single module touched by matching changed paths against the Architecture section of `CLAUDE.md`. Read that section inline.

**Gate**: Diff fetched, single module identified, context read.

## Phase 2: Stage 1 — Correctness (single reviewer)

> **Reviewer model**: `worker-reviewer` launch this tier use resolved `--reviewer` overlay value (tier=low default `haiku`; escalate to `sonnet` when structural markers from `classify.md` present). See `overlays.md` reviewer axis.
...
- Repo pattern compliance (grim envelope handling, lit-html-only DOM, escaping, scope-flag ownership)
