...
- Architecture section of `CLAUDE.md` for every touched area
- Relevant ADR (`.claude/artifacts/adr_*.md`) if one covers diff topic
- `CLAUDE.md` conventions — diff may imply UX/contract shifts review must catch
...
- **1** `worker-reviewer` (focus: `spec-compliance`, phase: `post-implementation`) — reviews **Implement**-phase output against repo anchors
...
- `worker-researcher` — SOTA gap: how do leading VS Code extensions / marketplace UIs solve same problem? Algorithm choice current? Known pitfalls unaddressed?
