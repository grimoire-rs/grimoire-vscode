...
Classifier (`classify.md`) decides _when_ to apply overlay from diff metrics, paths, PR signals. This file defines _what each axis means_ and pipeline effect.
...
- low → `haiku` (→ `sonnet` when structural markers from `classify.md` fire: `src/grim.ts`, `src/installer.ts`, webview escaping/CSP paths, `package.json` dependency changes, public API breakage)
- high → `sonnet`
