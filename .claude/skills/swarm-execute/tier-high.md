...
Read plan artifact from `.claude/artifacts/`. Parse classification (Scope, Reversibility, Tier, Overlays), Stub/Specify/Implement/Review phases, Subsystems Touched. Read the Architecture section of `CLAUDE.md` for all touched areas.

**Gate**: Plan steps parsed; architecture context read.

## Phase 2: Stub

Launch **1** `worker-builder` (focus: `stubbing`, model: sonnet) to create type signatures, traits, function shells with `throw new Error('unimplemented')`. No business logic.

**Gate**: `npm run check` passes (types compile).
...
**Gate**: `npm run check` succeeds. Run the fast check during loop — NOT full `task verify`.
