...
1. `/swarm-plan "add a sort dropdown to the sidebar"` → tier=**low**, no overlays, confident.
2. `/swarm-plan "refactor the snapshot/refresh pipeline"` → tier=**high** + overlays `--architect=opus` (cross-subsystem), `--codex` (public API). Likely promoted to `max` by `--architect=opus` + cross-subsystem combo — depends on prompt details.
3. `/swarm-plan "extend search caching"` → low-confidence (split between `low` — "extend" — and `high` — "caching layer change"). Meta-plan gate fires.
