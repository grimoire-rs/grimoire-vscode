...
1. **Read relevant quality rules FIRST, before any writes.** Path-scoped, auto-load by file type: `.claude/rules/quality-core.md` (universal, always loaded), plus language leaf (`quality-typescript.md`, `quality-bash.md`) matching files edited. Post-completion self-review no substitute.
2. **Grep for existing utilities before writing new code.** Check `src/webview/model.ts` (view-model helpers), `src/grim.ts` (argv builders + envelope types), `src/scopes.ts`, and `src/webview/render.ts` with Grep.
...
6. **Flag product-level insights.** Research/architecture/implementation uncovers a shift in extension positioning, UX direction, or grim CLI contract implications → flag in the completion report for human decision. Applies especially to `worker-researcher`, `worker-architect`, `worker-doc-writer`.
...
Canonical contract-first TDD protocol: Stub → Verify → Specify → Implement → Review-Fix Loop. `/swarm-execute` skill has the full detailed protocol incl. review-fix loop spec.
